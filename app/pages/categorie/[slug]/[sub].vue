<template>
  <div class="cat-page">
    ...
    <div class="cat-header">
      <div class="wrap">
        <nav class="breadcrumb">
          ...
          <NuxtLink :to="`/categorie/${slug}`">{{ category?.name || slug }}</NuxtLink>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"/></svg>
          <span>{{ subcategory?.name || sub }}</span>
        </nav>
        <h1 class="cat-title">{{ subcategory?.name || sub }}</h1>
    ...
    <div v-if="category?.subcategories?.length" class="sub-tabs-bar">
      <div class="wrap">
        <div class="sub-tabs">
          <NuxtLink :to="`/categorie/${slug}`" class="sub-tab">Tout</NuxtLink>
          <NuxtLink
            v-for="s in category.subcategories"
            :key="s.slug"
            :to="`/categorie/${slug}/${s.slug}`"
            class="sub-tab"
            :class="{ 'sub-tab--active': s.slug === sub }"
          >{{ s.name }}</NuxtLink>
        </div>
      </div>
    </div>
    ...
    <div class="wrap grid-section">
      ...
      <div v-else-if="announcements.length === 0" class="empty-state">
        ...
        <p>Aucune annonce dans cette sous-catégorie.</p>
        <NuxtLink :to="`/categorie/${slug}`" class="back-home">← Voir tout {{ category?.name }}</NuxtLink>
      </div>
...
const subcategory = computed(() => category.value?.subcategories.find(s => s.slug === sub.value) ?? null)

useHead(() => ({ title: `${subcategory.value?.name || sub.value} | meubletn` }))
...
async function load(page = 1) {
  ...
  try {
    ...
    if (subcategory.value) q.subcategory = subcategory.value.id
    else if (category.value) q.category = category.value.id
...
onMounted(() => load())
watch(sub, () => { searchQ.value = ''; conditionF.value = ''; load(1) })
</script>
...
.cat-header { background: #fff; border-bottom: 1px solid #f3f4f6; padding: 24px 0 20px; }
.breadcrumb { display: flex; align-items: center; gap: 6px; font-size: 12px; color: #9ca3af; margin-bottom: 10px; }
...
.sub-tabs-bar { background: #fff; border-bottom: 1px solid #f3f4f6; position: sticky; top: 0; z-index: 10; }
.sub-tabs { display: flex; gap: 4px; overflow-x: auto; scrollbar-width: none; padding: 10px 0; }
.sub-tabs::-webkit-scrollbar { display: none; }
.sub-tab { white-space: nowrap; padding: 6px 16px; border-radius: 20px; border: 1.5px solid #e5e7eb; background: #fff; font-size: 13px; font-weight: 500; color: #374151; text-decoration: none; transition: all 0.15s; flex-shrink: 0; }
.sub-tab:hover { border-color: #db3a1b; color: #db3a1b; }
.sub-tab--active { background: #db3a1b; border-color: #db3a1b; color: #fff; }
.toolbar { background: #fff; border-bottom: 1px solid #f3f4f6; padding: 10px 0; }
...
.search-input { width: 100%; height: 38px; border: 1.5px solid #e5e7eb; border-radius: 8px; padding: 0 12px 0 36px; font-size: 14px; outline: none; box-sizing: border-box; transition: border-color 0.15s; }
.search-input:focus { border-color: #db3a1b; }
.filters { display: flex; gap: 8px; flex-wrap: wrap; }
.filter-select { height: 38px; border: 1.5px solid #e5e7eb; border-radius: 8px; padding: 0 12px; font-size: 13px; background: #fff; color: #374151; outline: none; cursor: pointer; transition: border-color 0.15s; }
...
.skeleton-card { border-radius: 12px; background: linear-gradient(90deg, #f3f4f6 25%, #e9eaec 50%, #f3f4f6 75%); background-size: 200% 100%; animation: shimmer 1.4s infinite; aspect-ratio: 3/4; }
@keyframes shimmer { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }
.empty-state { text-align: center; padding: 80px 0; display: flex; flex-direction: column; align-items: center; gap: 12px; color: #9ca3af; font-size: 14px; }
.back-home { margin-top: 8px; color: #db3a1b; font-weight: 600; text-decoration: none; font-size: 14px; }
.back-home:hover { text-decoration: underline; }
.pagination { display: flex; align-items: center; justify-content: center; gap: 16px; padding: 40px 0 0; }
.page-btn { padding: 9px 20px; border: 1.5px solid #e5e7eb; border-radius: 8px; background: #fff; font-size: 14px; font-weight: 600; color: #374151; cursor: pointer; transition: all 0.15s; }
